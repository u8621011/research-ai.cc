  ...
  return (
    <section className={styles.research} id="research">
      <div className={styles.container}>
        <h2 className={styles.sectionTitle}>AI 輔助研究 · 精選案例</h2>
        <p className={styles.sectionSubtitle}>從數據整理到洞察生成，AI 實戰案例全公開</p>
        <div className={styles.researchGrid}>
          {researchCases.map((item, index) => (
            <div key={index} className={styles.researchCard}>
              <div className={styles.researchCardImage}>
                <img src={item.image} alt={item.title} className={styles.researchCardScreenshot} />
              </div>
              <div className={styles.researchCardContent}>
                <h3>{item.title}</h3>
                <p>{item.description}</p>
                <a href={item.link} className={styles.researchCardLink}>
